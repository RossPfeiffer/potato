...
function uploadBatchOfParts(position){
	...
	function TX(tx){
		tx.send({ from:machine.address, gasLimit:25000000 }, function(r,hash){
			if(r) throw r;
			...
			function readHash(){
				setTimeout(function(){
					...
					.then(function(res){
						//
						if(res === null){
							console.log("Nothing yet, trying again...")
							readHash();	
						}else{
							if(res.status){
